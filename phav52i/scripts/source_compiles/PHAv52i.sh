...
echo "IN PHAv52i.sh comptype [ $comptype ]"
# set the option for FAST or DBUG compilation
if [ $comptype = "DBUG" ]
  then
    copt="-g -fbounds-check -std=f95"
  if [ "$lf95opt" -eq "1" ]
    then
   copt="-g --trap --chk --chkglobal  -std=f95"
  fi  
#   copt="--chk s "
elif [ $comptype = "FAST" ]
  then 
#  set copt = "-O2 -funroll-loops -std=f95"
  copt="-O2 "
else
  echo "Invalid comtype: $comptype - STOPPING"
  exit  
fi
echo "IN PHAv52i.sh comptype [ $comptype ] [$copt ]"
