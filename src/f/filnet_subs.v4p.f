...
c               instead of state directories                   13may09
c             AND IN THE GHCN branch - GHCN-D and GHCN-Mv2
c               COMPATIBILITY (11 char name & 3 flags)         08jun09
c
...
C     GET LIST OF reference STATION IDS, and datafile names
   ...
   20   format(a11)
        if(inet .gt. NUMREF) then
          print *, ' ** Number of input stations greater than Array ***'
          print *, ' ***** Increase NUMREF in posthomog.parm.incl *****'
          stop
        endif  
        cstn = rid(inet)
...
c       the command line
        if(inet .le. nc) then
c          nfname(inet) = candir(1:lnblnk(candir)) // cstn(1:2) // '/' //
c     *      cstn // icelem(1:lnblnk(icelem)) // ctype(1:lnblnk(ctype))
c          ifname(inet) = candir(1:lnblnk(candir)) // cstn(1:2) // '/' //
c     *      cstn // icelem(1:lnblnk(icelem)) // itype(1:lnblnk(itype))
          nfname(inet) = candir(1:lnblnk(candir)) // 
     *      ctype(1:lnblnk(ctype)) // '/' // cstn // '.' // 
     *       ctype(1:lnblnk(ctype)) // '.' // icelem(1:lnblnk(icelem))
          ifname(inet) = candir(1:lnblnk(candir)) //
     *      itype(1:lnblnk(itype)) // '/' // cstn // '.' // 
     *       itype(1:lnblnk(itype)) // '.' // icelem(1:lnblnk(icelem))
        else
c          nfname(inet) = netdir(1:lnblnk(netdir)) // cstn(1:2) // '/' //
c     *      cstn // icelem(1:lnblnk(icelem)) // ntype(1:lnblnk(ntype))
c          ifname(inet) = netdir(1:lnblnk(netdir)) // cstn(1:2) // '/' //
c     *      cstn // icelem(1:lnblnk(icelem)) // itype(1:lnblnk(itype))
          nfname(inet) = netdir(1:lnblnk(netdir)) //
     *      ntype(1:lnblnk(ntype)) // '/' // cstn // '.' // 
     *       ntype(1:lnblnk(ntype)) // '.' // icelem(1:lnblnk(icelem))
   ...
   35 nstn = inet - 1
      print *,' Number of Input Stations: ', nstn
...
C     CLOSE network station FILE
  ...
  150 call perror(' Error in opening netfile: ' // netfile)
      stop 999
      
  170 call perror(' Error in reading netfile: ' // netfile)
      stop 999
...
c     delm will contain the network data
      ...
      character*11 cstn, cnstn/'           '/, instn, dummy
...
C ------ lobotomized from filnet's subroutine infil
...
c       in main
  ...
  110   FORMAT(60(I11,1X))
        read(nnunit, '(a11)',end=200) dummy
      enddo  
...
c       Open temporary i/o unit for data input
        open(idunit, file = nfname(ptr(K)), status = 'old', err = 175)
        do while (1 .eq. 1)
c          original HCN format
c          read(idunit, '(a6,1x,i1,1x,i4,12f8.2)', end = 100)
c     *      instn, isrc, iyear, indata
          read(idunit, '(a11,i1,i4,12(i6,a3))', end = 100)
     *      instn, isrc, iyear, (indata(imth),inflag(imth),imth=1,nmth)
          if(iyear .ge. IBYEAR .and. iyear .le. IEYEAR) then
            do imth = 1, NMTH
              if(indata(imth) .eq. IMISS) then
                delm(k, iyear - IBYEAR + 1, imth) = AMISS
c              else if(ikeep .eq. 0 .and.
c     *          inflag(imth)(2:2) .eq. 'd') then
c                delm(k, iyear - IBYEAR + 1, imth) = AMISS    
              else if(inflag(imth)(2:2) .ne. ' ') then
                delm(k, iyear - IBYEAR + 1, imth) = AMISS                
              else  
                delm(k,iyear-IBYEAR+1,imth) = indata(imth)/scale(inel)
              endif  
            end do
          end if
        end do
c       make sure to close this unit!!!
  ...
  175 call perror('CANNOT OPEN FILE:' // nfname(ptr(K)) )
      print *,'K, ptr(k):', K, ptr(k)
      igood = 0
      return
...
c     cdata is the candidate station's data 
      ...
      integer ngap(NMTH)
      character*3 dflag(nmth, inumyr), cflag(nmth,nyr)
...
c           because the Reference Network (HCN) is prefect :)
            do is = 2, inmstn
              nby(is) = ly
              nay(is) = nyr - ly
            end do  
...
c               if Original is missing - set data flag to E
c                if(dflag(im, ly)(2:2) .eq. ' ')dflag(im, ly)(1:1) = 'E'
c               Flag = 'E' is unconditional      change 23 Dec 2014
                dflag(im, ly)(1:1) = 'E'
                dint = dconf(im,ly)
...
C             NEIGHBORS MUST HAVE AT LEAST SEVEN YEARS; ETC. per de post-cw
...
C           DEBUG PRINTOUT
     ...
     *         dconf(im,ly),STALST
  150       FORMAT(a11,I5,i3,1X,F6.2,1X,2(I4,1X),2(F6.2,1X),
     *         3(F6.2,1X),A40)
...
C     DEBUG PRINTOUT
   ...
   40   FORMAT('BANF= ',60i6,/,'CONF= ',60F6.3/,
     *    'ISRT= ',60I6,/,'NAY = ',60I6,/,
     *    'NBY = ',60I6)
...
c          onum = 0.
c          do it = 1, ik
c            onet = onet + del(is,ly) / conf(is)
c            onum = onum + 1. / conf(is)
c          end do
c          print *, '>>', ik, bi/bn + onet/onum, wci
...
C           COMPUTE OPTIMIZED WEIGHTED AVERAGE
  ...
  120       CONTINUE
     ...
     *        OPCCF + OPNET / OPNUM
          END IF
c         no need to beat a dead horse, if there is no hope - STOP!
...
c      call hsort(tsdif, iban)

c      DO 30 IW2 = 1,iBAN-1
c        MIN = IW2

c        DO 20 IW1 = IW2+1,iBAN
c          IF(XSDIF(IW1) .LT. XSDIF(MIN)) MIN = IW1
c   20   CONTINUE

C     ORDER PRECIPITATION RATIOS

c        IF(MIN .NE. IW2) THEN
c          XSDTMP = XSDIF(IW2)
c          XSDIF(IW2) = XSDIF(MIN)
c          XSDIF(MIN) = XSDTMP
c        END IF
