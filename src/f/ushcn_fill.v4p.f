...
c         parameter lists in subroutines as well
      ...
      character*11 cstn
      character*256 argv, basedir/''/, netfile/''/, candfile/''/,
     *  reffile/''/
...
c ***********************************************************
      ...
      character*256 dfile, cfile, outdir
...
c      open(iounit, file=cfile)
      
c     write the data in normals format
c      do iy = 1, ieyear - ibyear + 1
c        ndat = 0
c        do im = 1, nmth
c          if(data(im,iy) .lt. amiss + 1.0 ) then
c            idval(im) = -9999
c          else
c            ndat = ndat + 1
c            idval(im) = nint(dconf(im,iy)*scale(inel)) 
c          endif
c        enddo  
          
c        if(ndat .gt. 0) then
c          write(iounit,1000,err=300) cstn, inel, iy + ibyear - 1,
c     *      (idval(im),cflag(im,iy), im=1,nmth)
c        endif  
c      enddo
