...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare per-station files across two directories"
    )
    ...
    parser.add_argument(
        "--no-mmap", action="store_true", help="Disable memory-mapped file reads"
    )
    ...
    while left_parsed is not None and right_parsed is not None:
        ...
        if left_year == right_year:
            ...
            for m in range(12):
                ...
                total_sum, total_sq, total_count = update_stats(
                    total_sum, total_sq, total_count, diff
                )
                bin_index = (year - 1700) // 30
                bin_sums[bin_index], bin_sqs[bin_index], bin_counts[bin_index] = (
                    update_stats(
...
def update_stats(
    sum_diff: float, sum_sq: float, count: int, diff: float
) -> Tuple[float, float, int]:
...
def main() -> int:
    ...
    def mean_and_rms(
        sum_diff: float, sum_sq: float, count: int
