...
program TOBMain
  ...
  logical            :: tob_backfill_if_first_nonblank
  logical            :: tob_pause_on_blank
  ...
  logical :: tob_blank(0:MAX_CHANGES)
  ...
  data_type   = get_property_chars(PROP_TOB_INPUT_DATA_TYPE)
...
contains
  ...
  90 format(i1,12x,2(1x,i4,2i2),1x,f3.0,2f3.0,1x,f4.0,2f3.0, &
    ...
    do
      ...
      if (obcode /= prev_code) then
        ...
        bdayh(nchgs)    = beg_day
        tobtmp(nchgs)   = obcode
        tob_blank(nchgs) = (len_trim(obs_time) == 0)
        prev_code     = obcode
      end if
    end do
  ...
  subroutine resolve_special_codes()
    ...
    paused = .false.
    do n = 1, nchgs
      if (tob_pause_on_blank) then
        if (tob_blank(n)) then
          if (.not. paused) then
            if (n > 1 .and. .not. tob_blank(n-1)) then
              paused = .true.
            end if
          end if
          if (paused) then
  ...
  function get_tob_start_year() result(start_year)
    ...
    do n = 1, nchgs
      if (tobtmp(n) >= 1 .and. tobtmp(n) <= 24) then
        if (tob_backfill_if_first_nonblank .and. n == 1 .and. .not. tob_blank(1)) then
          tobtmp(0) = tobtmp(1)
          return
        end if
        start_year = byrh(n)
